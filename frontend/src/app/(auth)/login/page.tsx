...
export default function LoginPage() {
  ...
  // Google-style: Show loading skeleton while checking auth
  // Prevents flash of login form when user is already authenticated
  if (authLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4 py-10 bg-[#f0f4f9] dark:bg-[#111111]">
        <div className="w-full max-w-[450px] bg-white dark:bg-[#1e1e1e] border border-[#dadce0] dark:border-[#3c4043] rounded-[28px] px-10 py-10 sm:px-12">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="h-20 bg-gray-100 dark:bg-gray-800 rounded animate-pulse" />
            <div className="h-12 bg-gray-200 dark:bg-gray-700 rounded-full animate-pulse mt-1" />
          </div>
        </div>
      </div>
    )
  ...
  // If user is authenticated, redirect immediately (no flash)
  if (user) {
