...
from academics.models import Organization, Program, Batch, Subject
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            programs_without_batches = Program.objects.filter(
                organization=org
            ).annotate(
            ...
            if total_programs == 0:
                self.stdout.write(self.style.SUCCESS("✅ All programs already have batches!"))
                return
                        ...
                        for year in years:
                            batch, created = Batch.objects.get_or_create(
                                ...
                                defaults={
                                    ...
                                    'current_semester': min(((current_year - year) * 2) + 1, program.total_semesters),
                                    'total_students': 0,
                                    'is_active': True
                                }
                            )
                            if created:
                                ...
                                for subject in subjects:
                                    from academics.models import BatchSubjectEnrollment
                                    BatchSubjectEnrollment.objects.get_or_create(
                                        organization=org,
                                        batch=batch,
                                        subject=subject,
            ...
            # Print summary
            self.stdout.write("\n" + "="*80)
            programs_with_batches = Program.objects.filter(
                organization=org
            ).annotate(
                batch_count=Count('batches')
            ).filter(
