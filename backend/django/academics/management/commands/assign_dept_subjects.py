    ...
    def handle(self, *args, **options):
        with transaction.atomic():
            ...
            for faculty in faculties:
                ...
                has_subjects = Subject.objects.filter(faculty_assigned=faculty.faculty_id).exists()
                
                if not has_subjects:
                    # Get subjects from same department
                    dept_subjects = Subject.objects.filter(department=faculty.department).exclude(
                        faculty_assigned__in=Subject.objects.filter(
                            faculty_assigned__isnull=False
                        ).values_list('faculty_assigned', flat=True).distinct()
                    ).first()
                    ...
                    if dept_subjects:
                        ...
                        faculty.save()
                        updated_count += 1
                        self.stdout.write(f'✓ {faculty.faculty_name} ({faculty.department.department_name}) -> {dept_subjects.subject_name}')
                else:
                    # Update specialization for those who have subjects
                    subjects = Subject.objects.filter(faculty_assigned=faculty.faculty_id)[:2]
