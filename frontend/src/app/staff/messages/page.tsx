...
export default function MessagesPage() {
  const messages = [
    { id: 1, from: 'Dr. Smith (CS Dept)', subject: 'Lab 3 Booking Conflict', message: 'There seems to be a double booking for Lab 3 on Friday 2-4 PM. Can we resolve this?', time: '2 hours ago', priority: 'high', type: 'conflict', unread: true },
    { id: 2, from: 'Prof. Johnson (Math)', subject: 'Schedule Change Request', message: 'Need to swap Tuesday and Thursday slots for Calculus II due to conference attendance.', time: '4 hours ago', priority: 'medium', type: 'request', unread: true },
    { id: 3, from: 'Maintenance Team', subject: 'Auditorium AC Repair', message: 'AC repair scheduled for Monday 10 AM - 2 PM. Please block this slot.', time: '1 day ago', priority: 'high', type: 'maintenance', unread: false },
    { id: 4, from: 'Admin Office', subject: 'New Faculty Onboarding', message: 'Dr. Wilson joins EE department next week. Please allocate teaching slots.', time: '2 days ago', priority: 'low', type: 'info', unread: false }
  ]

  const departments = ['Computer Science', 'Electronics', 'Mathematics', 'Physics', 'Mechanical', 'Admin Office', 'Maintenance']
          ...
          <div className="lg:col-span-2">
            <div className="card">
              ...
              <div className="space-y-3">
                {messages.map((msg) => (
                  <div key={msg.id} className={`p-3 sm:p-4 rounded-lg border-l-4 ${
                    msg.type === 'conflict' ? 'bg-red-50 dark:bg-red-900/20 border-red-500' :
                    msg.type === 'request' ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-500' :
                    msg.type === 'maintenance' ? 'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-500' :
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      ...
                      <button className="btn-ghost text-xs px-3 py-1.5 flex-1 sm:flex-none">Mark Read</button>
                    </div>
                  </div>
                ))}
              </div>
