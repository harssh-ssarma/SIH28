...
# Import multi-tenant models (these are the primary models now)
from .multi_tenant_models import (
    Organization,
    Campus,
    School,
    Department as MTDepartment,
    Program,
    Subject as MTSubject,
    Faculty as MTFaculty,
    FacultySubject,
    Batch as MTBatch,
    Student as MTStudent,
    Classroom as MTClassroom,
    TimeSlot,
    BatchSubjectEnrollment,
    StudentElectiveChoice,
    User,  # Multi-tenant User model (replaces old User)
    TimetablePreferences,
)

# Create your models here.
# NOTE: The User model is now imported from multi_tenant_models.py
# The old single-tenant models below are deprecated and will be removed in a future version

# DEPRECATED: Old single-tenant User model (now using multi-tenant version from multi_tenant_models.py)
