...
export default function ApprovalsPage() {
  const approvals = [
    { id: 1, title: 'CS Semester 5 Timetable', department: 'Computer Science', submittedBy: 'Dr. Smith', date: '2024-03-18', status: 'pending', conflicts: 2, students: 120 },
    { id: 2, title: 'Math Semester 3 Schedule', department: 'Mathematics', submittedBy: 'Prof. Johnson', date: '2024-03-17', status: 'approved', conflicts: 0, students: 80 },
    { id: 3, title: 'Physics Lab Schedule', department: 'Physics', submittedBy: 'Dr. Brown', date: '2024-03-16', status: 'rejected', conflicts: 5, students: 60 },
    { id: 4, title: 'EE Semester 4 Practical', department: 'Electronics', submittedBy: 'Prof. Wilson', date: '2024-03-18', status: 'validation', conflicts: 1, students: 90 }
  ]
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="card">
          <div className="card-header">
            ...
            <p className="card-description text-xs sm:text-sm">Review and validate timetable submissions</p>
          </div>
              ...
              <div key={approval.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-3 sm:p-4 bg-neutral-50 dark:bg-neutral-800 rounded-lg">
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-2">
                    ...
                    <span className={`badge text-xs self-start ${
                      ...
                      approval.status === 'validation' ? 'badge-info' : 'badge-danger'
                    }`}>
                      {approval.status}
                    </span>
                  </div>
                ...
                <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                  ...
                  {(approval.status === 'pending' || approval.status === 'validation') && (
                    <>
                      <button className="btn-success text-xs px-3 py-1.5 flex-1 sm:flex-none">Approve</button>
                      <button className="btn-danger text-xs px-3 py-1.5 flex-1 sm:flex-none">Reject</button>
                    </>
                  )}
                </div>
              </div>
