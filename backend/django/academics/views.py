...
class SubjectViewSet(SmartCachedViewSet):
    ...
    filterset_fields = ["department", "program", "semester", "subject_type"]
...
class FacultyViewSet(DataSyncMixin, PerformanceMetricsMixin, SmartCachedViewSet):
    ...
    def get_model_stats(self, queryset):
        ...
        try:
            return {
                "by_department": {
                    str(dept): queryset.filter(department__dept_id=dept).count()
                    for dept in queryset.values_list(
                        "department__dept_id", flat=True
                    ).distinct()
                    if dept
                },
                "avg_teaching_hours": queryset.aggregate(
                    Avg("max_teaching_hours_per_week")
                )["max_teaching_hours_per_week__avg"]
                or 0,
            }
...
class StudentViewSet(DataSyncMixin, PerformanceMetricsMixin, SmartCachedViewSet):
    ...
    def get_model_stats(self, queryset):
        ...
        try:
            return {
                "by_year": {
                    year: queryset.filter(current_year=year).count()
                    for year in range(1, 5)
                },
                "by_semester": {
                    sem: queryset.filter(current_semester=sem).count()
                    for sem in range(1, 9)
                },
                "by_program": {
                    str(prog): queryset.filter(program__program_id=prog).count()
                    for prog in queryset.values_list(
...
class ClassroomViewSet(SmartCachedViewSet):
    ...
    filterset_fields = ["department", "campus", "room_type", "is_available"]
...
def dashboard_stats_view(request):
    ...
    Returns only counts and essential data for admin dashboard
    """
    try:
        user = request.user
        ...
        # Get counts efficiently without loading full data
        stats = {
            "total_users": User.objects.filter(organization_id=org_id).count(),
            "active_courses": Program.objects.filter(
                organization_id=org_id, is_active=True
            ).count(),
            "total_faculty": Faculty.objects.filter(organization_id=org_id).count(),
            "total_students": Student.objects.filter(organization_id=org_id).count(),
            "pending_approvals": 0,  # Placeholder
            "system_health": 98,
        }

        # Get 3 sample faculty for display
        sample_faculty = Faculty.objects.filter(organization_id=org_id).select_related(
            "department"
        )[:3]
            ...
            faculty_data.append(
                {
                    "id": str(f.faculty_id),
                    "name": f.faculty_name,
                    "department": f.department.dept_name if f.department else "N/A",
                    "isAvailable": f.is_available,
                }
            )

        return Response({"stats": stats, "faculty": faculty_data})
