...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            # Get CS department
            cs_dept = Department.objects.filter(department_name__icontains='computer').first()
            mgmt_dept = Department.objects.filter(department_name__icontains='management').first()
