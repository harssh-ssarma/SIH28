...
from academics.models import (
    User, Student, Faculty, Subject, Batch, Department, Course
)
from academics.attendance_models import (
    SubjectEnrollment, AttendanceSession, AttendanceRecord, AttendanceAlert
)
import random
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not students:
            self.stdout.write(self.style.ERROR('No students found! Run import_csv_data first.'))
            return
        
        # Get sample faculty (first 3)
        faculties = list(Faculty.objects.all()[:3])
        if not faculties:
            self.stdout.write(self.style.ERROR('No faculty found! Run import_csv_data first.'))
            return
        
        # Get sample subjects (first 5)
        subjects = list(Subject.objects.all()[:5])
        if not subjects:
            self.stdout.write(self.style.ERROR('No subjects found! Run import_csv_data first.'))
            return
        
        # Get sample batch
        batch = Batch.objects.first()
        if not batch:
            self.stdout.write(self.style.ERROR('No batch found! Run import_csv_data first.'))
            return
        
        self.stdout.write(f'Found {len(students)} students, {len(faculties)} faculty, {len(subjects)} subjects')
        ...
        for student in students:
            for subject in subjects[:3]:  # Enroll each student in 3 subjects
                enrollment, created = SubjectEnrollment.objects.get_or_create(
                    ...
                    semester=student.current_semester if hasattr(student, 'current_semester') else 1,
                    defaults={
                        'is_active': True,
                        'batch': batch
                    }
            ...
            for subject in random.sample(subjects, min(3, len(subjects))):
                ...
                session, created = AttendanceSession.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        'is_marked': True,
                        'marked_at': timezone.make_aware(start_time + timedelta(minutes=10)),
        ...
        self.stdout.write(self.style.SUCCESS(f'Created {record_count} attendance records'))
        ...
        for student in students[:3]:
            ...
            if user:
                ...
                self.stdout.write(f'    Name: {student.name} | ID: {student.student_id}')
        ...
        for faculty in faculties[:2]:
            ...
            if user:
                ...
                self.stdout.write(f'    Name: {faculty.name} | ID: {faculty.faculty_id}')
