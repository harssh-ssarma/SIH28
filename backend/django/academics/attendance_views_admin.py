...
class FacultyAttendanceViewSet(viewsets.ViewSet):
    ...
    @action(detail=False, methods=['get'], url_path='class-report/(?P<subject_id>[^/.]+)')
...
class AdminAttendanceViewSet(viewsets.ViewSet):
    ...
    def audit_logs(self, request):
        ...
        logs = AttendanceAuditLog.objects.all().select_related(
            'record', 'changed_by'
        ).order_by('-changed_at')
    ...
    def generate_report(self, request):
        ...
        report_data['summary']['average_attendance'] = round(
...
class AttendanceAlertViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role in ['admin', 'staff']:
            return AttendanceAlert.objects.all().select_related(
