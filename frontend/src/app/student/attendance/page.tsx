            ...
            >
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-gray-900 dark:text-white">{courseData.total_classes}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 dark:text-gray-400">Present:</span>
                  <span className="font-medium text-green-600 dark:text-green-400">{courseData.present_classes}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 dark:text-gray-400">Absent:</span>
                  <span className="font-medium text-red-600 dark:text-red-400">{courseData.total_classes - courseData.present_classes}</span>
                </div>
              </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          ...
          <div className="card text-center">
            <div className="text-2xl sm:text-3xl font-bold text-green-600 dark:text-green-400">
              {Object.values(attendanceData).reduce((sum, course) => sum + course.present_classes, 0)}
            </div>
            <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">Present</div>
          </div>
          <div className="card text-center">
            <div className="text-2xl sm:text-3xl font-bold text-red-600 dark:text-red-400">
              {Object.values(attendanceData).reduce((sum, course) => sum + (course.total_classes - course.present_classes), 0)}
            </div>
          ...
          <div className="card text-center">
            <div className="text-2xl sm:text-3xl font-bold text-purple-600 dark:text-purple-400">
              {Object.values(attendanceData).length > 0 
                ? Math.round(Object.values(attendanceData).reduce((sum, course) => sum + course.percentage, 0) / Object.values(attendanceData).length)
                : 0}%
            </div>
