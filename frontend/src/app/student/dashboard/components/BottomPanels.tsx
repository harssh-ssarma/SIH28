          ...
          <div key={index} className="p-3 rounded-lg" style={{ background: 'var(--color-bg-surface-2)' }}>
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <h4 className="text-xs sm:text-sm font-medium leading-tight" style={{ color: 'var(--color-text-primary)' }}>{assignment.title}</h4>
                <p className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>{assignment.subject} ({assignment.code})</p>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mt-2">
                  <p className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>Due: {assignment.due}</p>
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm leading-relaxed" style={{ color: 'var(--color-text-primary)' }}>{notification.message}</p>
                <p className="text-xs mt-1" style={{ color: 'var(--color-text-muted)' }}>{notification.time}</p>
              </div>
            </div>
          </div>
        ))}
  ...
  return (
    <div className="card lg:col-span-2 xl:col-span-1">
      ...
      <div className="space-y-4">
        <div>
          <h4 className="text-xs sm:text-sm font-medium mb-2" style={{ color: 'var(--color-text-primary)' }}>Recent Materials</h4>
          <div className="space-y-2">
            {[
              ...
              <div key={index} className="flex items-center gap-3 p-2 rounded-lg" style={{ background: 'var(--color-bg-surface-2)' }}>
                <div className="text-sm flex-shrink-0">
                  {material.type === 'PDF' ? '\u{1F4DD}' : material.type === 'PPT' ? '\u{1F4CA}' : '\u{1F4C4}'}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium truncate" style={{ color: 'var(--color-text-primary)' }}>{material.title}</p>
                  <p className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>{material.course} &bull; {material.size}</p>
                </div>
        ...
        <div>
          <h4 className="text-xs sm:text-sm font-medium mb-2" style={{ color: 'var(--color-text-primary)' }}>Anonymous Feedback</h4>
          <div className="space-y-2">
            ...
            <button className="w-full p-3 rounded-lg text-left transition-colors" style={{ background: 'var(--color-bg-surface-2)', border: '1px solid var(--color-border)' }} onMouseEnter={e => (e.currentTarget.style.background = 'var(--color-bg-surface-3)')} onMouseLeave={e => (e.currentTarget.style.background = 'var(--color-bg-surface-2)')}>
              <div className="flex items-center gap-2">
                <span className="text-sm">\u2605</span>
                <div>
                  <p className="text-xs sm:text-sm font-medium" style={{ color: 'var(--color-text-primary)' }}>Faculty Performance</p>
                  <p className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>Rate teaching quality and methods</p>
                </div>
              </div>
            </button>
          </div>
