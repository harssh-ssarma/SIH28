        ...
        with transaction.atomic():
            ...
            for dept in departments:
                ...
                if not dept_faculties or not dept_subjects:
                    self.stdout.write(f'  Skipped (faculties: {len(dept_faculties)}, subjects: {len(dept_subjects)})')
                    continue
                
                self.stdout.write(f'  Faculties: {len(dept_faculties)}, Subjects: {len(dept_subjects)}')
                ...
                for subject in dept_subjects:
                    ...
                    min_count = min(faculty_subject_count.values()) if faculty_subject_count else 0
                    available_faculties = [f for f in dept_faculties if faculty_subject_count[f.faculty_id] == min_count or f.faculty_id not in faculty_subject_count]
                    
                    if available_faculties:
                        faculty = available_faculties[faculty_idx % len(available_faculties)]
                        subject.faculty_assigned = faculty.faculty_id
                        subject.save()
                        faculty_subject_count[faculty.faculty_id] += 1
                        self.stdout.write(f'  ✓ {subject.subject_name} -> {faculty.faculty_name}')
                        faculty_idx += 1
                
                # Update faculty specializations
                for faculty in dept_faculties:
                    assigned = Subject.objects.filter(faculty_assigned=faculty.faculty_id)
                    if assigned.exists():
